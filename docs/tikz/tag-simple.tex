...
\node[state,fill=white] (Processor){Processor};
\node[state,dashed,fill=white] (SenseLow)[below=of Processor] {Low $I_q$ Sensors};
\node[state,dashed,fill=white] (FlashLow)[below=of SenseLow] {Low $I_q$ External Flash};
\node[state,fill=white] (RTC)[above=of Processor] {RTC};
\node[fill=none](Label2)[below=of FlashLow,draw=none]{Always On};
...
\node[state,dashed,fill=white] (SenseHigh)[left=2cm of SenseLow] {High $I_q$ Sensors};
...
\begin{pgfonlayer}{bg2}
  \node[state,inner sep=10pt] (Switched)
       ...
       [fit=(SenseHigh)(Label1),fill=blue!10,thick]{};
\end{pgfonlayer}


\node[state,fill=white](Battery)[right=2cm of SenseLow] {Battery};
%\node[state,dashed,fill=white](Regulator)[below=of Battery]{Regulator};
\node[state,fill=none](Label3)[below=of Battery,draw=none]{Power};
\begin{pgfonlayer}{bg2}
  \node[state,inner sep=10pt](Power)
       %[fit=(Battery)(Regulator)(Label3),fill=blue!10, thick]{};
       [fit=(Battery)(Label3),fill=blue!10, thick]{};
\end{pgfonlayer}  
